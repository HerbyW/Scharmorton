...
var checkHE = maketimer(0.2, func
{ 
  if (getprop("fdm/jsbsim/buoyant_forces/gas-cell/initial-temp") == 0)
  {  
   var mas = getprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[0]/contents-mol") + getprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[1]/contents-mol");
   setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]", mas / -16.2 );
  }
});
 ...
 if (getprop("/fdm/jsbsim/fcs/mooring2-cmd-norm") == 1)
    {
      ...
      gui.popupTip("Filling Helium to -3% for take off in progress. " ~
                 "Current weight " ~ weight_on_gear.getValue() ~ ".");
    var wowmol = (weight_on_gear.getValue() * -13.5);
    var cont0 = getprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[0]/contents-mol");
    var cont1 = getprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[1]/contents-mol");
    interpolate("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[1]/contents-mol", cont1 + 0.93 * wowmol, 10);
    interpolate("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[0]/contents-mol", cont0 + 0.07 * wowmol, 10);
  ...
  if (getprop("/fdm/jsbsim/buoyant_forces/gas-cell/volume-ft3") > 529500.0) setprop("/fdm/jsbsim/fcs/takeoffweight", 0);
}
...
setlistener("/fdm/jsbsim/fcs/blower-valve-cmd-norm/trigger/state", func(state)
{
  if(state.getValue())
  {
    if(getprop("/fdm/jsbsim/animation/hull") > 0.99)
    {
      ...
      setprop("sim/messages/copilot", "Hot Air Ballon is filled");
    }
     else
     {
      var mol = getprop("/fdm/jsbsim/buoyant_forces/gas-cell/contents-mol") + 1000;
...
setlistener("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]", func {
  if (getprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]") < -21000  )
  {
  setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]", -21000 );
  setprop("/sim/messages/copilot", "Maximum Helium in!");
  }
  if (getprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]") > 0  )
  {
  setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[2]", 0 );
  setprop("/sim/messages/copilot", "No Helium in!");
  setprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[0]/contents-mol", 0);
  setprop("/fdm/jsbsim/buoyant_forces/gas-cell/ballonet[1]/contents-mol", 0);
  }
});

####################################################################################
#Butan control
#         /fdm/jsbsim/fcs/flame-valve-cmd-norm    Flame 0 1 2 3


var consum = maketimer(1.0, func
{ 
 propane = getprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]"); 
 if (getprop("/fdm/jsbsim/fcs/flame-valve-cmd-norm") == 1 )
 {
   setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]", propane - 0.232626 );
 }
 if (getprop("/fdm/jsbsim/fcs/flame-valve-cmd-norm") == 2 )
 {
   setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]", propane - 0.465251 );
 }
 if (getprop("/fdm/jsbsim/fcs/flame-valve-cmd-norm") == 3 )
 {
   setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]", propane - 0.697877 );
 }
});
...
setlistener("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]", func {
  if (getprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[0]") < 24  )
  {
  setprop("/sim/messages/copilot", "Less than 2% Propane on board!");
...
########################################################################################################
# Rip Valve control
#    /fdm/jsbsim/fcs/rip-cmd-norm       0 oder 100
...
{ 
 if (ripped == 1 and getprop("/fdm/jsbsim/animation/hull") < 0.15 )
 {
   rip_panel = 0;
   setprop("/fdm/jsbsim/fcs/rip-cmd-norm", 0);
   setprop("/sim/messages/copilot", "Ballon is ripped and nearly empty!");
 }
});
...
#################################################################
    create : func {
        ...
        props.globals.initNode("sim/about/text",
             "Scharmorton Hot Air Ballon Version 3.2 for FlightGear\n" ~
             "Copyright (C) 2016 Herbert Wagner\n\n" ~
